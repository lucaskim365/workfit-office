...
interface AuthState {
  ...
  /** 프로필 자기 수정(이메일, 인감 URL). 저장 후 세션 user 상태도 최신화. */
  updateProfile: (patch: { email?: string; sealUrl?: string }) => Promise<void>;
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function updateProfile(patch: { email?: string; sealUrl?: string }) {
    if (!user) throw new Error('로그인 상태가 아닙니다.');
