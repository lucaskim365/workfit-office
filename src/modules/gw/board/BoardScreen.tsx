...
const BOARDS: Board[] = [
  ...
  { id: 'event', name: '경조사', icon: '💐', desc: '임직원들의 소식을 함께 나눕니다.' },
...
export default function BoardScreen() {
  ...
  return (
    <div className="flex h-full w-full gap-5 bg-panel p-6 text-[12.5px] text-ink">
          ...
          {BOARDS.map((b) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3.5 rounded-lg px-3.5 py-3 text-left font-bold transition-all ${isActive
      ...
      {/* ── 우측 메인 영역 ── */}
      <main className="flex-1 flex flex-col gap-4 rounded-xl border border-border bg-panel p-5 shadow-sm">
        {/* 상단 타이틀 & 헤더 */}
      ...
      {selectedPost && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-2xl rounded-xl border border-border bg-panel shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
            {/* 모달 헤더 */}
            <div className="flex items-center justify-between border-b border-border bg-panel-alt/40 p-4">
              <span className="font-extrabold text-teal">
                {BOARDS.find((b) => b.id === selectedPost.boardId)?.name || '게시판'} 상세
              </span>
              <button
                onClick={() => setSelectedPost(null)}
                className="text-lg text-ink3 hover:text-ink transition-colors"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              <div className="space-y-2 border-b border-border pb-3">
                <h2 className="text-sm font-extrabold text-ink leading-snug">
              ...
              {/* 본문 내용 */}
              <div className="whitespace-pre-wrap leading-relaxed text-[12.5px] py-2 border-b border-border/60 pb-4 text-ink">
                {selectedPost.content}
              </div>

              {/* 첨부파일 영역 */}
              {selectedPost.hasAttachment && (
                <div className="rounded-lg border border-border bg-panel-alt/30 p-3 space-y-2">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="space-y-2">
                  {MOCK_COMMENTS.map((c) => (
                    <div key={c.id} className="rounded-lg border border-border bg-panel-alt/10 p-3 flex flex-col gap-1.5">
                ...
                <div className="flex gap-2 pt-1">
                  <input
                    ...
                    className="flex-1 h-9 rounded-lg border border-border-hi bg-panel px-3 text-[11.5px] outline-none focus:border-teal"
                  />
                  <button
            ...
            {/* 모달 풋터 */}
            <div className="border-t border-border bg-panel-alt/20 p-3.5 flex justify-end gap-2">
              <button
                onClick={() => setSelectedPost(null)}
      ...
      {isWriteModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-xs p-4">
          <form
            ...
            className="w-full max-w-xl rounded-xl border border-border bg-panel shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
          >
            {/* 모달 헤더 */}
            <div className="flex items-center justify-between border-b border-border bg-panel-alt/40 p-4">
              <span className="font-extrabold text-teal">✍️ 게시글 작성</span>
              <button
                type="button"
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-3.5">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  className="h-9 w-full rounded-lg border border-border-hi bg-panel px-2 outline-none focus:border-teal"
                >
                  {BOARDS.map((b) => (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  required
                  className="h-9 w-full rounded-lg border border-border-hi bg-panel px-3 outline-none focus:border-teal"
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  required
                  rows={8}
                  className="w-full rounded-lg border border-border-hi bg-panel p-3 outline-none focus:border-teal resize-y leading-relaxed"
                />
              </div>
            ...
            <div className="border-t border-border bg-panel-alt/20 p-3.5 flex justify-end gap-2">
              ...
              <button
                type="submit"
                className="rounded-lg bg-teal px-4 py-2 font-bold text-white hover:opacity-90"
              >
                등록
              </button>
            </div>
