...
import { LEAVE_TYPES, type ApprovalDoc, type ApprovalStep, type LeaveForm, type LeaveType } from '@/domain/approvalDoc/schema';
...
import { DynamicField, missingRequired, daysBetween } from '@/modules/gw/approval/formFields';
...
export function ApprovalDraftModal({
  ...
  const [values, setValues] = useState<Record<string, FieldValue>>(editDoc?.fieldValues ?? {});
  ...
  const buildInput = (): ApprovalDraftInput => {
    const leave: LeaveForm | null = isLeave ? { leaveType, startDate, endDate, days: leaveDays } : null;
    return {
      docType: code,
      title: title.trim(),
      drafterId: me.id,
  ...
  for (const field of form?.fields ?? []) {
    ...
    if (field.type === '금액' && field === amountField) {
      ...
      fieldNodes.push(<div key={field.key} className="col-span-2"><Field label={field.label}>
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={4} placeholder={field.placeholder || '내용을 입력하세요'} className={`${INP} resize-none leading-relaxed`} />
      </Field></div>);
    } else {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
          ...
          {isLeave && (
            <div className="grid grid-cols-3 gap-2">
              ...
              <Field label="시작일"><input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className={INP} /></Field>
              <Field label={`종료일 (${leaveDays}일)`}><input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className={INP} /></Field>
            </div>
          )}
