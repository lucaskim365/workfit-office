...
import { fmtDateTime, StatusBadge } from '@/modules/gw/_gw';
import { PlusCircle } from 'lucide-react';

/** 진행중 문서의 현재 결재자 이름(표기용). */
...
export function MyLeaveTab() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-panel p-4 shadow-sm">
        ...
        {canCreate && (
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-teal px-4 py-2 text-[12px] font-bold text-white shadow-sm transition-all hover:opacity-90 shrink-0"
          >
            <PlusCircle size={15} />
            <span>휴가 신청하기</span>
          </button>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="relative overflow-hidden rounded-xl border border-border bg-panel p-5 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="w-full bg-border rounded-full h-2 overflow-hidden mb-3">
            <div
              ...
              style={{ width: `${bal.grant > 0 ? (bal.remaining / bal.grant) * 100 : 0}%` }}
            />
          </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-border bg-panel p-5 shadow-sm transition-all hover:shadow-md">
          ...
          <div className="flex justify-between text-[11px] text-ink3 font-medium pt-1 border-t border-border/50">
            ...
            {bal.substituteHoliday.pending > 0 && <span className="text-amber font-bold">결재 대기: {bal.substituteHoliday.pending}일</span>}
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-border bg-panel p-5 shadow-sm transition-all hover:shadow-md">
          ...
          {upcomingDoc ? (
            ...
            <div className="flex flex-col items-center justify-center py-4 text-center">
              <span className="text-[16px] font-bold text-ink3">일정 없음</span>
      ...
      {bal.otherUsed > 0 && (
        <p className="text-[11px] text-ink3">※ 병가·경조·공가 등 기타 완료 휴가 {bal.otherUsed}일은 연차 잔여와 별개로 차감되지 않습니다.</p>
      )}
      ...
      <div className="flex border-b border-border">
        <button
          ...
          className={`px-5 py-2.5 text-[12.5px] font-bold transition-all border-b-2 -mb-[2px] ${
      ...
      <div className="rounded-xl border border-border bg-panel overflow-hidden shadow-sm">
        ...
        {activeTab === 'annual' && (
          <div className="p-4">
            <div className="flex justify-between items-center mb-3">
              <span className="text-[12.5px] font-bold text-ink">연차 변동 상세 내역</span>
        ...
        {activeTab === 'substitute' && (
          <div className="p-4">
            <div className="flex justify-between items-center mb-3">
              <span className="text-[12.5px] font-bold text-ink">대체휴무 발생 및 만료 상세</span>
