...
export default function QualIqcWaitScreen() {
  ...
  const urgent = lots.filter((l) => l.due === '지연' || (l.due === 'D-Day' && l.status !== '검사중')).length;
  const waiting = lots.filter((l) => l.status === '대기');
  const avgWait = waiting.length ? (waiting.reduce((s, l) => s + l.wait, 0) / waiting.length).toFixed(1) : '0.0';

  if (!cur) {
    return <div className="grid place-items-center py-20 text-[13px] text-ink3">{isLoading ? '불러오는 중…' : '수입검사 대상이 없습니다.'}</div>;
  }
      ...
      <div className="grid grid-cols-1 items-start gap-3.5 lg:grid-cols-[1.85fr_1fr]">
        ...
        <Card title="입고 LOT 상세" bodyClassName="p-0" action={<Pill tone={ST_TONE[cur.status]}>{cur.status}</Pill>}>
            ...
            <div className="flex flex-col gap-1.5">
              {cur.items.map((it, i) => (
                <div key={i} className="flex items-center gap-2 rounded-[7px] px-2.5 py-1.5" style={{ background: C.panelAlt }}>
                  <span className="w-[26px] shrink-0 text-[9px] font-bold" style={{ color: it.type === '계량' ? C.blue : C.teal }}>{it.type}</span>
                  <span className="shrink-0 text-[11px] font-bold text-ink">{it.name}</span>
                  <span className={`ml-auto text-right text-[10px] text-ink3 ${it.type === '계량' ? 'font-mono' : ''}`}>{it.spec}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-2 px-4 py-3.5">
            <button
              onClick={() => {
                if (cur.status === '대기') transition.mutate({ recv: cur.recv, to: '검사중' });
                else if (cur.status === '보류') transition.mutate({ recv: cur.recv, to: '검사중' });
              }}
            ...
            >
              {cur.status === '대기' ? '검사 착수 →' : cur.status === '보류' ? '검사 재개 →' : '검사 실적 등록 →'}
            </button>
            <button
