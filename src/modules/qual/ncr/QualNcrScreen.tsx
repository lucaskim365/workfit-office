...
import { useNonconformances, useTransitionNcr } from '@/features/nonconformance/useNonconformances';
import { nextStatus, nextActionLabel } from '@/domain/nonconformance/status';
...
export default function QualNcrScreen() {
  const { data: list = [], isLoading } = useNonconformances();
  const transition = useTransitionNcr();
  const [sel, setSel] = useState('NCR-260621-008');
  const [tab, setTab] = useState('전체');
  const cur = list.find((n) => n.no === sel) || list[0];
  const rows = list.filter((n) => tab === '전체' || (tab === '미종결' ? n.status !== '종결' : n.status === '종결'));

  const open = list.filter((n) => n.status !== '종결').length;
  const invest = list.filter((n) => n.status === '조사중').length;
  const isoQty = list.filter((n) => n.status !== '종결').reduce((s, n) => s + n.iso, 0);

  if (!cur) {
    return <div className="grid place-items-center py-20 text-[13px] text-ink3">{isLoading ? '불러오는 중…' : '부적합 보고서가 없습니다.'}</div>;
  }

  const advanceLabel = nextActionLabel(cur.status);
