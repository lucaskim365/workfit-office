        ...
        <Card title="품목 목록" action={<span className="text-[10.5px] text-ink3">총 248건 중 {rows.length}건</span>} bodyClassName="p-0">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[12px]">
              ...
              <tbody>
                {isLoading && (
                  <tr><td colSpan={7} className="px-3 py-8 text-center text-[11.5px] text-ink3">불러오는 중…</td></tr>
                )}
                {!isLoading && rows.length === 0 && (
                  <tr><td colSpan={7} className="px-3 py-8 text-center text-[11.5px] text-ink3">조회된 품목이 없습니다.</td></tr>
                )}
                {rows.map((it) => {
          ...
          {cur ? (
            <div className="flex flex-col gap-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <DetailField label="안전재고" value={`${cur.safetyStock.toLocaleString()} ${cur.unit}`} mono />
              </div>
              <DetailField label="사용여부" select value={cur.use} />
              <DetailField label="비고" multiline value={cur.remark} />
            </div>
          ) : (
            <div className="py-10 text-center text-[11.5px] text-ink3">선택된 품목이 없습니다.</div>
          )}
        </Card>
      </div>
    </div>
