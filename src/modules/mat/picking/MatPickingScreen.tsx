...
const ROWS: string[][] = [
  ['1', 'WF-300-B', 'LOT-RAW-8810', 'A-1-1-1', '2026-05-20', '2,500', 'FIFO'],
  ['2', 'CHM-SL-05', 'LOT-CHM-0440', 'A-3-1-1', '2026-05-28', '40', 'FIFO'],
  ['3', 'RES-PR-22', 'LOT-RES-1102', 'A-3-2-2', '2026-06-01', '20', 'FIFO'],
];
            ...
            <tbody>
              {ROWS.map((r, i) => (
                <tr key={i} style={{ background: i === 0 ? C.tealSoft : i % 2 ? C.panelAlt : '#fff' }} className="cursor-pointer">
                  <td className={`${td('center')} font-extrabold tabular-nums`} style={{ color: C.navy, borderLeft: i === 0 ? `3px solid ${C.teal}` : '3px solid transparent' }}>{r[0]}</td>
                  <td className={`${td('left')} font-mono text-[11px] font-semibold text-ink`}>{r[1]}</td>
                  <td className={`${td('left')} font-mono text-[11px] font-bold`} style={{ color: C.teal }}>{r[2]}</td>
                  <td className={td('left')}><span className="rounded-[5px] px-2 py-0.5 font-mono text-[10.5px] text-ink2" style={{ background: C.bgDeep }}>{r[3]}</span></td>
                  <td className={`${td('left')} tabular-nums text-ink3`}>{r[4]}</td>
                  <td className={`${td('right')} font-bold tabular-nums text-ink`}>{r[5]}</td>
                  <td className={td('center')}><Pill tone="info">{r[6]}</Pill></td>
                </tr>
              ))}
            </tbody>
          </table>
