...
import { useMolds } from '@/features/mold/useMolds';
...
export default function MoldMasterScreen() {
  const { data: molds = [], isLoading } = useMolds();
  ...
  if (!cur) {
    return <div className="grid place-items-center py-20 text-[13px] text-ink3">{isLoading ? '불러오는 중…' : '등록된 자산이 없습니다.'}</div>;
  }
