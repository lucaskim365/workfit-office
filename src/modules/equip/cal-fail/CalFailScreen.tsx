...
import { useCalFails } from '@/features/calFail/useCalFails';
...
export default function CalFailScreen() {
  const { data: rows = [], isLoading } = useCalFails();
  const [sel, setSel] = useState('CAL-2305-007');
  const cur = rows.find((r) => r.sn === sel) || rows[0];

  const noAction = rows.filter((r) => r.dispo === '미조치').length;
  const stopped = rows.filter((r) => r.dispo === '사용중지').length;
  const repair = rows.filter((r) => r.dispo === '수리신청').length;
  const scrap = rows.filter((r) => r.dispo === '폐기').length;

  if (!cur) {
    return <div className="grid place-items-center py-20 text-[13px] text-ink3">{isLoading ? '불러오는 중…' : '불합격 계측기가 없습니다.'}</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <KpiGrid cols={5} items={[
        ['미조치', '' + noAction, '대', C.err], ['사용 중지', '' + stopped, '대', C.warn], ['수리 신청', '' + repair, '대', C.blue], ['폐기 처리', '' + scrap, '대', C.ink], ['총 불합격', '' + rows.length, '대', C.ink],
      ]} />
