...
import { useEquipGages } from '@/features/equipGage/useEquipGages';
...
export default function GageMasterScreen() {
  const { data: GAGES = [], isLoading } = useEquipGages();
  ...
  if (!cur) {
    return <div className="grid place-items-center py-20 text-[13px] text-ink3">{isLoading ? '불러오는 중…' : '등록된 계측기가 없습니다.'}</div>;
  }
