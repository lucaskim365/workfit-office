...
import { useSpareScraps, useTransitionSpareScrap } from '@/features/spareScrap/useSpareScraps';
...
export default function SpareScrapScreen() {
  const { data: rows = [], isLoading } = useSpareScraps();
  const transition = useTransitionSpareScrap();
  const [sel, setSel] = useState('SP-PMP-DM');
  const [filter, setFilter] = useState('전체');

  if (isLoading) {
    return <div className="px-3 py-8 text-center text-sm text-ink3">불러오는 중…</div>;
  }
  if (rows.length === 0) {
    return <div className="px-3 py-8 text-center text-sm text-ink3">등록된 불용/폐기 항목이 없습니다.</div>;
  }

  const shown = rows.filter((r) => filter === '전체' || r.state === filter);
  const cur = rows.find((r) => r.code === sel) || rows[0];
  const curIdx = flowIndex(cur.state);
  const rejected = cur.state === '반려';

  const cntObsolete = rows.filter((r) => r.state === '불용지정').length;
  const cntWait = rows.filter((r) => r.state === '승인대기').length;
  const cntApproved = rows.filter((r) => r.state === '승인완료').length;
  const scrapAmt = rows.filter((r) => r.state === '폐기완료').reduce((s, r) => s + r.book * r.qty, 0);
  const recoverAmt = rows.filter((r) => r.state !== '반려').reduce((s, r) => s + r.recover * r.qty, 0);

  /** 상태 전이 실행(채번된 항목만 — no='–'는 미채번이라 전이 불가). */
  const go = (to: SpareScrap['state']) => {
    if (cur.no === '–') return;
    transition.mutate({ no: cur.no, to });
  };
      ...
      <div className="grid grid-cols-1 items-start gap-3.5 lg:grid-cols-[1fr_330px]">
        ...
        <Card title="처리 내역" bodyClassName="p-0" action={<span className="font-mono text-[10.5px] text-ink3">{cur.no}</span>}>
          ...
          <div className="flex flex-col gap-2 px-4 py-3.5">
            {cur.state === '불용지정' && <button onClick={() => go('폐기요청')} className="w-full rounded-[9px] py-3 text-[13px] font-extrabold text-white" style={{ background: C.navy }}>폐기 승인 요청</button>}
            {cur.state === '승인대기' && (
              <div className="flex gap-2">
                <button onClick={() => go('승인완료')} className="flex-1 rounded-[9px] py-3 text-[13px] font-extrabold text-white" style={{ background: C.teal }}>승인</button>
                <button onClick={() => go('반려')} className="flex-1 rounded-[9px] border bg-panel py-3 text-[13px] font-extrabold" style={{ borderColor: C.err, color: C.err }}>반려</button>
              </div>
            )}
            {cur.state === '승인완료' && <button onClick={() => go('폐기완료')} className="w-full rounded-[9px] py-3 text-[13px] font-extrabold text-white" style={{ background: C.navy }}>폐기 완료 처리</button>}
            {cur.state === '반려' && <button onClick={() => go('폐기요청')} className="w-full rounded-[9px] py-3 text-[13px] font-extrabold text-white" style={{ background: C.navy }}>재상신</button>}
