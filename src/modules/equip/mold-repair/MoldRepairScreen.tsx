...
interface Row { no: string; date: string; code: string; name: string; type: string; detail: string; shotAt: number; hrs: number; cost: number; vendor: string; who: string; state: string }
const REP_ROWS: Row[] = [
...
export default function MoldRepairScreen() {
  ...
  const shown = REP_ROWS.filter((r) => filter === '전체' || (filter === '세척' ? r.type === '세척' || r.type === '정기세척' : r.type === filter));

  const repairCnt = REP_ROWS.filter((r) => r.type === '수리').length;
  const ongoing = REP_ROWS.filter((r) => r.state === '진행중').length;
  const totalCost = REP_ROWS.reduce((s, r) => s + r.cost, 0);
  ...
  return (
    <div className="flex flex-col gap-3.5">
      ...
      <KpiGrid cols={5} items={[
        ['당월 작업', '' + REP_ROWS.length, '건', C.ink], ['수리 건', '' + repairCnt, '건', C.err], ['진행중', '' + ongoing, '건', C.warn],
