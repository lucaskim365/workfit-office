...
import { documentExecutionRepo, subscribeExecutions } from '@/data/documentExecution/documentExecution.repo';
import type { DocumentExecution } from '@/domain/documentExecution/schema';
...
const EXEC_KEY = 'documentExecutions';

/** 모든 시행 임무 구독 조회 훅 */
export function useAllExecutions() {
  const qc = useQueryClient();
  const [data, setData] = useState<DocumentExecution[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    const fetchAndSubscribe = () => {
      documentExecutionRepo.list().then((list) => {
        setData(list);
        qc.setQueryData([EXEC_KEY], list);
        setIsLoading(false);
      });
    };

    fetchAndSubscribe();
    const unsub = subscribeExecutions(fetchAndSubscribe);
    return unsub;
  }, [qc]);
...
/** 부서별 대기/진행 중인 시행 목록 조회 훅 (부서 시행함) */
export function useDeptExecutions(deptId: string | undefined) {
  const q = useAllExecutions();
  return useMemo(() => {
    if (!deptId) return [];
    return q.data.filter((e) => e.targetDeptId === deptId && e.status !== 'CANCELLED');
  }, [q.data, deptId]);
}

/** 특정 문서에 연결된 복수 시행 목록 조회 훅 */
export function useDocumentExecutions(docId: string | undefined) {
  const q = useAllExecutions();
  return useMemo(() => {
    if (!docId) return [];
    return q.data.filter((e) => e.documentId === docId);
  }, [q.data, docId]);
}

/** 내가 접수한 시행 업무 목록 조회 */
export function useMyClaimedExecutions(userId: string | undefined) {
  const q = useAllExecutions();
  return useMemo(() => {
    if (!userId) return [];
    return q.data.filter((e) => e.assigneeId === userId);
  }, [q.data, userId]);
}

/** 시행 담당자로 나 지정하기 (내가 담당하기) */
export function useClaimExecution() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ executionId, userId, userName }: { executionId: string; userId: string; userName: string }) =>
      documentExecutionRepo.claimExecution(executionId, userId, userName),
    onSuccess: () => {
...
/** 시행 담당자 지정/변경 (부서장용) */
export function useAssignExecutorV2() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({
      executionId,
      ...
      documentExecutionRepo.assignExecutor(executionId, executorId, executorName, actorId, actorName),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
...
/** 시행 담당 업무 반납 */
export function useReleaseExecution() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ executionId, userId, userName }: { executionId: string; userId: string; userName: string }) =>
      documentExecutionRepo.releaseExecution(executionId, userId, userName),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
...
/** 시행 완료 보고 */
export function useCompleteExecutionV2() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({
      executionId,
      ...
      documentExecutionRepo.completeExecution(executionId, userId, userName, completedAt, comment),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
...
/** 시행 불가 및 반송 */
export function useReturnExecution() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({
      executionId,
      ...
      documentExecutionRepo.returnExecution(executionId, userId, userName, comment, reasonType),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
...
/** 보완 후 재상신 */
export function useResubmitExecution() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({
      executionId,
      ...
      documentExecutionRepo.resubmitExecution(executionId, userId, userName, comment),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
...
export function useCancelExecution() {
  ...
  return useMutation({
    mutationFn: ({ executionId, userId, userName }: { executionId: string; userId: string; userName: string }) =>
      documentExecutionRepo.cancelExecution(executionId, userId, userName),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: [EXEC_KEY] });
      qc.invalidateQueries({ queryKey: [KEY] });
    },
