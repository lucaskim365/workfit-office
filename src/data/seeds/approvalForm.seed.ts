...
const SEED_RAW: any[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      f({ key: 'leaveType', label: '휴가종류', type: '선택', required: true, width: 'half', options: ['연차', '오전반차', '오후반차', '반차', '반반차', '병가', '경조', '공가', '대체휴무'] }),
      f({ key: 'substituteId', label: '업무대행자(대결자)', type: '사용자', width: 'half', placeholder: '업무 인수인계자/대결자 선택' }),
      f({ key: 'emergencyContact', label: '긴급 연락처', type: '텍스트', required: true, width: 'half', placeholder: '예: 010-1234-5678' }),
  ...
  },
  {
    ...
    fields: [
      f({ key: 'outsideType', label: '외근 구분', type: '선택', required: true, width: 'half', options: ['종일 외근', '오전 외근', '오후 외근', '시간 외근'] }),
      f({ key: 'period', label: '외근 일자/기간', type: '기간', required: true, width: 'half' }),
      f({ key: 'startTime', label: '외근 시작 시각', type: '텍스트', width: 'half', placeholder: '예: 14:00 (종일이면 비움)' }),
      f({ key: 'endTime', label: '외근 복귀 시각', type: '텍스트', width: 'half', placeholder: '예: 18:00 (종일이면 비움)' }),
      f({ key: 'destination', label: '외근지(방문처)', type: '텍스트', required: true, width: 'half', placeholder: '예: 판교 고객사' }),
      f({ key: 'transportation', label: '교통수단', type: '선택', width: 'half', options: ['대중교통', '자차', '회사차량', '도보/기타'] }),
      f({ key: 'substituteId', label: '업무대행자', type: '사용자', width: 'half', placeholder: '부재 시 업무대행자' }),
      f({ key: 'emergencyContact', label: '긴급 연락처', type: '텍스트', width: 'half', placeholder: '예: 010-1234-5678' }),
...

  {
    ...
    fields: [
      f({ key: 'destination', label: '출장지', type: '텍스트', required: true, width: 'half', section: '출장 정보', placeholder: '예: 부산 지사/고객사' }),
      f({ key: 'purpose', label: '목적', type: '선택', required: true, width: 'half', section: '출장 정보', options: ['영업', '기술지원', '교육', '회의', '프로젝트', '기타'] }),
      f({ key: 'period', label: '출장 기간', type: '기간', required: true, width: 'full', section: '출장 정보' }),
      f({ key: 'companions', label: '동행자', type: '텍스트', width: 'half', section: '출장 정보', placeholder: '동행자 성명 (없으면 비움)' }),
      f({ key: 'transportation', label: '교통수단', type: '선택', width: 'half', section: '출장 정보', options: ['KTX/기차', '고속버스', '항공', '자차', '회사차량', '기타'] }),
      f({ key: 'substituteId', label: '업무대행자(대결자)', type: '사용자', width: 'half', section: '출장 정보', placeholder: '출장 중 업무 인수인계자' }),
      f({ key: 'emergencyContact', label: '긴급 연락처', type: '텍스트', width: 'half', section: '출장 정보', placeholder: '예: 010-1234-5678' }),
      f({ key: 'amount', label: '예상 경비', type: '금액', required: true, isAmountKey: true, width: 'full', section: '비용 정보', placeholder: '교통비·숙박비·일비 합계' }),
  ...
  },
  {
    ...
    fields: [
      f({ key: 'overseasType', label: '해외출장 구분', type: '선택', required: true, width: 'half', section: '출장 정보', options: ['일반 해외출장', '비자 및 여권 발급', '현지 실사/전시회'] }),
      f({ key: 'destination', label: '출장지(국가/도시)', type: '텍스트', required: true, width: 'half', section: '출장 정보', placeholder: '예: 미국 샌프란시스코' }),
      f({ key: 'purpose', label: '목적', type: '선택', required: true, width: 'half', section: '출장 정보', options: ['영업', '기술지원', '교육', '회의', '전시회 참가', '기타'] }),
      f({ key: 'period', label: '출장 기간', type: '기간', required: true, width: 'full', section: '출장 정보' }),
      f({ key: 'companions', label: '동행자', type: '텍스트', width: 'half', section: '출장 정보', placeholder: '동행자 성명 (없으면 비움)' }),
      f({ key: 'substituteId', label: '업무대행자(대결자)', type: '사용자', width: 'half', section: '출장 정보', placeholder: '출장 중 업무 인수인계자' }),
      f({ key: 'emergencyContact', label: '비상 연락처', type: '텍스트', width: 'half', section: '출장 정보', placeholder: '현지 로밍 연락처 또는 비상망' }),
      f({ key: 'amount', label: '예상 경비', type: '금액', required: true, isAmountKey: true, width: 'full', section: '비용 정보', placeholder: '항공권·숙박비·체재비 합계' }),
      f({ key: 'body', label: '상세 일정 및 계획', type: '장문', required: true }),
    ],
  },
  {
    ...
    fields: [
      ...
      f({
        ...
        required: false,
        width: 'half',
        options: ['간식대', '외근식대', '야근식대'],
        visibleIf: 'expenseItem:식대',
      }),
      f({
        key: 'meet_notice',
        label: '회의비는 10만원 이내 사용이 원칙입니다.',
        type: '안내문',
        required: false,
        width: 'full',
        visibleIf: 'expenseItem:회의비',
      }),
      f({
        key: 'sort_veh',
        label: '분류',
        ...
        required: true,
        width: 'half',
        options: ['야근 후 택시비', '외근 교통비', '외근 택시비'],
        visibleIf: 'expenseItem:교통비',
      }),
      f({
        key: 'sort_carr',
        label: '분류',
      ...
      f({
        ...
        required: false,
        width: 'half',
        options: ['부서원별(2개월한도액4만원)', '특별회식(전체회식)'],
        visibleIf: 'expenseItem:회식비',
      }),
      f({
        key: 'amount',
        label: '청구 금액',
