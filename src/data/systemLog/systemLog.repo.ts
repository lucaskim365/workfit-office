...
/**
 * 시스템 로그 Repository — Firestore 접근을 캡슐화하는 유일한 계층.
...
let memory: SystemLog[] = SYSTEM_LOG_SEED.map((it) => systemLogSchema.parse(it));
...
export const systemLogRepo = {
  ...
  async list(filter?: SystemLogFilter): Promise<SystemLog[]> {
    if (isFirebaseConfigured && db) {
      ...
      const rows = snap.docs.map((d) => systemLogSchema.parse(d.data()));
  ...
  async save(item: SystemLog): Promise<void> {
    const valid = systemLogSchema.parse(item);
    if (isFirebaseConfigured && db) {
