...
/**
 * 매출 Repository — Firestore 접근을 캡슐화하는 유일한 계층.
 * domain·features·UI 는 이 파일을 통해서만 데이터에 접근한다.
 * ([[DB_이관_대비_설계원칙.md]] 원칙 1: DB 교체 시 이 파일만 재작성)
 *
 * Firebase 미설정이면 in-memory seed 로 graceful degrade.
 */
const COLL = 'salesRevenues';
...
let memory: SalesRevenue[] = SALES_REVENUE_SEED.map((it) => salesRevenueSchema.parse(it));
...
export const salesRevenueRepo = {
  ...
  async list(filter?: SalesRevenueFilter): Promise<SalesRevenue[]> {
    if (isFirebaseConfigured && db) {
      ...
      const rows = snap.docs.map((d) => salesRevenueSchema.parse(d.data()));
  ...
  async save(item: SalesRevenue): Promise<void> {
    const valid = salesRevenueSchema.parse(item);
    if (isFirebaseConfigured && db) {
