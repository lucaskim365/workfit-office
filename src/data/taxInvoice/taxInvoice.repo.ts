...
/**
 * 세금계산서 Repository — Firestore 접근을 캡슐화하는 유일한 계층.
 * domain·features·UI 는 이 파일을 통해서만 데이터에 접근한다.
 * ([[DB_이관_대비_설계원칙.md]] 원칙 1: DB 교체 시 이 파일만 재작성)
 *
 * Firebase 미설정이면 in-memory seed 로 graceful degrade.
 */
const COLL = 'taxInvoices';
...
let memory: TaxInvoice[] = TAX_INVOICE_SEED.map((it) => taxInvoiceSchema.parse(it));
...
export const taxInvoiceRepo = {
  ...
  async list(filter?: TaxInvoiceFilter): Promise<TaxInvoice[]> {
    if (isFirebaseConfigured && db) {
      ...
      const rows = snap.docs.map((d) => taxInvoiceSchema.parse(d.data()));
  ...
  async save(item: TaxInvoice): Promise<void> {
    const valid = taxInvoiceSchema.parse(item);
    if (isFirebaseConfigured && db) {
